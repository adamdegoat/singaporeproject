...
def main():
    ...
    ap.add_argument("--emit-termini", action="store_true",
                    help="write an authored terminus at every mid-air end")
    a = ap.parse_args()
    ...
    # AN AUTHORED TERMINUS IS SOMEWHERE TO STOP — that is what it is for.
    #
    # But NOT while emitting them, or the pass is bistable: run once and the
    # ends acquire decks; run again and every end now "stops somewhere", so
    # zero are emitted, the termini list is overwritten empty, and the next run
    # finds all 34 dead ends back. A step that runs on every build has to
    # converge, so the emitter judges the world WITHOUT its own output.
    for t in ([] if a.emit_termini else (d.get("termini") or [])):
        q = t.get("p")
        if isinstance(q, (list, tuple)) and len(q) >= 2:
            stoppers.append((float(q[0]), float(q[1]), 8.0))
    ...
    # A PATH THAT ENDS AT NOTHING IS GIVEN SOMETHING TO END AT.
    #
    # The owner's rule is that nothing may just stop halfway. There are two
    # honest ways to satisfy it: connect the end to something (data/stitch.py
    # already does that wherever the gap is small enough to be a missing join),
    # or give the end a REASON — which is what a real path that stops has. A
    # trail ends at a lookout; a service spur ends at a turning head.
    #
    # So the ends that survive every exemption get an authored terminus, and
    # they stop being defects because they stop being ends at nothing. This is
    # written by the same pass that FINDS them, deliberately: a second
    # implementation of "which ends are mid-air" would drift from this one, and
    # then the world would grow decks where there are no dead ends and leave
    # dead ends where there are no decks.
    if a.emit_termini:
        out = []
        for (w, e) in midair:
            kind = "turn" if w["k"] in ("service", "road", "unclassified",
                                        "residential", "living_street") else "deck"
            out.append({"p": [round(e[0], 1), round(e[1], 1)], "k": kind,
                        "w": round(min(6.0, max(2.5, w.get("wide", 3.0))), 1)})
        # dedupe: two ways can end on the same spot
        seen, uniq = set(), []
        for t in out:
            key = (round(t["p"][0] / 3), round(t["p"][1] / 3))
            if key in seen:
                continue
            seen.add(key)
            uniq.append(t)
        d["termini"] = uniq
        json.dump(d, open(os.path.join(HERE, f"{a.id}.json"), "w"),
                  separators=(",", ":"))
        print(f"   wrote {len(uniq)} authored termini "
              f"({len(out) - len(uniq)} duplicate ends merged)")
